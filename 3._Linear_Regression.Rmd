...
sort(sqrt(vif(lm4)))
```
Now, it's better. Two variables - *dis* and *nox* - are edge cases, but we will keep them. You can try to build a new model (lm5) by excluding one of them (the one that is less associated with the outcome variable) and see if you'll get a better model.
